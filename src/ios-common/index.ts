...
import { exists, writeFile } from "../actions-common";
...
export async function installCertInTemporaryKeychain(
  ...
  p12CertPath: string,
  p12Pwd: string,
  useKeychainIfExists: boolean
): Promise<void> {
  let setupKeychain: boolean = true;

  if (useKeychainIfExists && exists(keychainPath)) {
    setupKeychain = false;
  }
  ...
  if (!setupKeychain) {
    const privateKeyName: string = await getP12PrivateKeyName(
      p12CertPath,
      p12Pwd
    );
 ...
 * @returns {string}
 */
export async function getCloudEntitlement(
  ...
  const cloudEntitlement = await execPlistBuddyCommand(
    "Print Entitlements:com.apple.developer.icloud-container-environment",
    tmpPlist
  );
 ...
 */
export async function installProvisioningProfile(
  provProfilePath: string
): Promise<{ provProfileUUID: string; provProfileName: string }> {
  ...
  if (provProfileUUID) {
    ...
    let pathToProvProfile: string = getProvisioningProfilePath(
      provProfileUUID,
      provProfilePath
    );

    core.debug(
      "copying provisioning profile to destination: " + pathToProvProfile
    );
...
export async function getProvisioningProfileName(provProfilePath: string) {
  //find the provisioning profile UUID
  let tmpPlist = "_xcodetasktmp.plist";
  let provProfileDetails = await saveProvisioningProfileDetails(
    provProfilePath,
    tmpPlist
  );
...
export async function getiOSProvisioningProfileType(provProfilePath: string) {
  ...
  try {
    ...
    let provisionsAllDevices = await execPlistBuddyCommand(
    ...
    if (
      ...
      provisionsAllDevices.trim().toLowerCase() === "true"
    ) {
      //ProvisionsAllDevices = true in enterprise profiles
      provProfileType = "enterprise";
    } else {
      let getTaskAllow = await execPlistBuddyCommand(
      ...
      if (getTaskAllow && getTaskAllow.trim().toLowerCase() === "true") {
        ...
        let provisionedDevices = await execPlistBuddyCommand(
...
export async function getDefaultKeychainPath() {
  ...
  getKeyChainOpts.listeners = {
    stdout: (data: Buffer) => {
      if (data) {
        defaultKeychainPath = data
          .toString()
          .trim()
          .replace(/[",\n\r\f\v]/gm, "");
        if (!exists(defaultKeychainPath)) {
 ...
 */
export async function getP12Properties(
  p12Path: string,
  p12Pwd: string
): Promise<{
  ...
  function onLine(line: string) {
    if (line) {
      ...
      if (key === "SHA1 Fingerprint") {
        ...
        const matches: string = value.split(",")[1]
        if (matches) {
 ...
 * @param p12Pwd Password for the P12 file
 */
export async function getP12PrivateKeyName(
  p12Path: string,
  p12Pwd: string
): Promise<string> {
  ...
  await exec.exec(opensslPathCmd, [
    "pkcs12",
    "-in",
    p12Path,
    "-out",
    "/tmp/p12privkeyname.tmp",
    "-nocerts",
    "-passin",
    "pass:" + p12Pwd,
    "-passout",
  ...
  await exec.exec(
    grepCmd,
    ["friendlyName", "/tmp/p12privkeyname.tmp"],
    grepOpts
  );
 ...
 */
async function setKeyPartitionList(
  keychainPath: string,
  keychainPwd: string,
  ...
  if (privateKeyName) {
    ...
    setKeyOpts.listeners = {
      errline: (line: string) => {
        if (
          !unknownCommandErrorFound &&
          line.includes("security: unknown command")
        ) {
...
function getProvisioningProfilePath(
  uuid: string,
  provProfilePath?: string
): string {
  ...
  return path.resolve(
    getUserProvisioningProfilesPath(),
    uuid.trim().concat(profileExtension)
  );
}
      ...
      stdout: (data: Buffer) => {
        if (data) {
          if (provProfileDetails) {
            provProfileDetails = provProfileDetails.concat(
              data
                .toString()
                .trim()
                .replace(/[,\n\r\f\v]/gm, "")
...
async function saveProvisioningProfileDetails(
  profProfilePath: string,
  writeToFile: string
) {
